...
function matrix.softmax(mtx)
    local max_val = -math.huge -- Find the maximum value in the matrix for numerical stability
    for i = 1, #mtx do
        for j = 1, #mtx[i] do
            if mtx[i][j] > max_val then
                max_val = mtx[i][j]
            end
        end
    end
    ...
    local exp_mtx = matrix.replace(mtx, function(x) return math.exp(x - max_val) end)
    
    -- Calculate the sum of each row for normalization
    local row_sums = {}
    for i = 1, #exp_mtx do
        row_sums[i] = 0
        for j = 1, #exp_mtx[i] do
            row_sums[i] = row_sums[i] + exp_mtx[i][j]
        end
    end
    ...
    for i = 1, #exp_mtx do
        for j = 1, #exp_mtx[i] do
            exp_mtx[i][j] = exp_mtx[i][j] / row_sums[i]
        end
    end
...
function matrix.relu(mtx)
    return matrix.replace(mtx, function(x) return math.max(0, x) end)
end

-- Tanh activation function
function matrix.tanh(mtx)
    return matrix.replace(mtx, math.tanh)
end
